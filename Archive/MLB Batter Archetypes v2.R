...
pc <- pca$x[,1:3] %>% as.data.frame() 

pc <- pc %>%
  mutate(PC1 = as.numeric(100 + 15*scale(PC1)*sd(pc[,"PC1"])),
         PC2 = as.numeric(100 + 15*scale(PC2)*sd(pc[,"PC2"])),
         PC3 = as.numeric(100 + 15*scale(PC3)*sd(pc[,"PC3"]))) %>%
  as.data.frame()
...
bind_rows(lapply(c("PC1", "PC2", "PC3"), function(col) 
  get_leaders(archetypes, col, 10, 2023))) %>%
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(  
      tabsetPanel(
        id = "tabs",
        tabPanel(title = uiOutput("title1"), 
                 fluidRow(
                   column(6, plotOutput("lineGraph1")),
                   column(6, tableOutput("bestMatches1"))),
                 fluidRow(
                   column(12, plotlyOutput("pie1")))),
        tabPanel(title = uiOutput("title2"), 
                 fluidRow(
                   column(6, plotOutput("lineGraph2")),
                   column(6, tableOutput("bestMatches2"))),
                 fluidRow(
                   column(12, plotlyOutput("pie2")))),
        tabPanel(title = "Player Comparison", 
                 fluidRow(
                   column(6, plotlyOutput("pieComp1")),
                   column(6, plotlyOutput("pieComp2"))),
...
server_pie <- function(input, output, session) {
  ...
  updateSelectizeInput(session, 'player1', 
                       choices = unique(archetypes$PlayerYear), 
                       server = TRUE)
  ...
  output$title1 = renderText({
    archetypes[archetypes$PlayerYear == input$player1, ncol(archetypes)] 
  })
  
  output$title2 = renderText({
    archetypes[archetypes$PlayerYear == input$player2, ncol(archetypes)]
  })
  ...
  output$pie1 <- renderPlotly({
    ...
    r <- percent(map_dbl(
      player_year_data[, grepl("probC", names(player_year_data))], ~.x))
  ...
  output$pie2 <- renderPlotly({
    player_year_data <- 
      archetypes[archetypes$PlayerYear == input$player2, ] %>% 
      mutate_at(vars(c(starts_with('probC'))), ~.*100)
  ...
  output$lineGraph1 <- renderPlot({
    ...
    ggplot(data, aes(x = year)) +
      ...
      labs(title = paste("PCs Over Time for", input$player1), 
           x = "Year", 
           y = "PC Value") +
      theme_minimal()
  })
  ...
  output$lineGraph2 <- renderPlot({
    data <- archetypes %>%
      filter(player_id == archetypes %>% 
               filter(PlayerYear == input$player2) %>% 
               select(player_id) %>%
               pull())
    ...
    ggplot(data, aes(x = year)) +
      ...
      geom_line(aes(y = PC3, color = "PC3")) +
      scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
      labs(title = paste("PCs Over Time for", input$player2), 
           x = "Year", 
           y = "PC Value") +
      theme_minimal()
  })
  ...
  # Player 1 Best Matches
  output$bestMatches1 <- renderTable({
    player <- archetypes %>%
      filter(PlayerYear == input$player1) %>%
      select(PC1, PC2, PC3)
    ...
    matches <- archetypes %>%
      filter(player_id != archetypes %>% 
               ...
               pull()) %>%
      rowwise() %>%
      mutate(match = calculate_match(PC1 - player$PC1, 
                                     PC2 - player$PC2, 
                                     PC3 - player$PC3)) %>%
      arrange(desc(match)) %>%
      head(5)
  ...
  # Player 2 Best Matches
  output$bestMatches2 <- renderTable({
    player <- archetypes %>%
      filter(PlayerYear == input$player2) %>%
      select(PC1, PC2, PC3)
...
cards <- list(
  ...
  card(
    full_screen = TRUE,
    card_header("Comparable Players"),
    uiOutput("comps")
  ),
  card(
    full_screen = TRUE,
    card_header("PC Scores over Time"),
...
ui <- page_sidebar(
  ...
  layout_columns(col_widths = c(5, 7, 12),
                 row_heights = c(2, 3),
                 cards[[2]], 
                 cards[[3]],
                 cards[[1]]
...
server <- function(input, output) {
  ...
  output$comps <- renderUI({
    ...
    datatable(matches, fillContainer = TRUE, 
